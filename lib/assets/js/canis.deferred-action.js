...
CanisInstructionHandler.prototype.handleDeferredAction = function() {
    var self = this;
    if (deferredNavItem) {
...
DeferredNavItem.prototype = jQuery.extend(true, {}, CanisComponent.prototype);
...
DeferredNavItem.prototype.setInteractionStatus = function(hasInteraction) {
    ...
    if (hasInteraction !== this.hasInteraction) {
        ...
        if (hasInteraction) {
            ...
            self.$link.stop(true, false).removeAttr('style');
        }
    }
};
...
DeferredNavItem.prototype.renderInteraction = function($canvas, item, interaction) {
    ...
    switch (interaction.inputType) {
        case 'select':
            $input = $('<select />', {'name': 'interaction-value', 'id': 'interaction-value'}).renderSelect(interaction.options.options, false, interaction.options.default);
            $input.change(function() {
                if ($(this).val() === 'selectObject' && $.fn.objectSelector !== undefined) {
                    var $newInput = $('<input />', {'type': 'hidden', 'name': 'interaction-value', 'id': 'interaction-value'});
                    $input.replaceWith($newInput);
                    $input = $newInput;
                    var $selectedText = $('<div />').hide().insertAfter($input);
                    $input.objectSelector({
                        'callback': function($selector, item) {
                            $input.val('*-' + item.id);
                            $selectedText.html('<strong>Selected</strong>: ' + item.descriptor).show();
                        }
                    });
                }
            });
