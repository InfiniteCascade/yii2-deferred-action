...
DeferredNavItem.prototype.handleData = function(data) {
	...
	jQuery.each(data.items, function(id, item) {
		...
		if (self.items[id] === undefined) {
			...
			self.items[id].$duration = $("<span />", {'class': 'label label-default'}).appendTo(self.items[id].$canvas);
			self.items[id].$status = $("<span />", {'class': 'label pull-right'}).appendTo(self.items[id].$canvas);
